...
/**
 * A specialized hook for the AnalyzeBankStatement BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 ...
 *
 * **Return Type:**
 * - **Non‑streaming:** types.CreditScoreAnalysis
 * - **Streaming Partial:** CreditScoreAnalysis
 * - **Streaming Final:** types.CreditScoreAnalysis
 *
 * **Usage Patterns:**
 ...
 * const { data, error, isLoading, mutate } = useAnalyzeBankStatement({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useAnalyzeBankStatement({
 ...
 * ```
 */
export function useAnalyzeBankStatement(props: HookInput<'AnalyzeBankStatement', { stream: false }>): HookOutput<'AnalyzeBankStatement', { stream: false }>
export function useAnalyzeBankStatement(props?: HookInput<'AnalyzeBankStatement', { stream?: true }>): HookOutput<'AnalyzeBankStatement', { stream: true }>
export function useAnalyzeBankStatement(
  props: HookInput<'AnalyzeBankStatement', { stream?: boolean }> = {},
): HookOutput<'AnalyzeBankStatement', { stream: true }> | HookOutput<'AnalyzeBankStatement', { stream: false }> {
  let action: ServerAction = Actions.AnalyzeBankStatement;
  if (isStreamingProps(props)) {
    action = StreamingActions.AnalyzeBankStatement;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'AnalyzeBankStatement', { stream: false }>)
  }
}
/**
 * A specialized hook for the ExtractTransactions BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 * - pdfText: string
 ...
 *
 * **Return Type:**
 * - **Non‑streaming:** types.BankTransaction[]
 * - **Streaming Partial:** BankTransaction[]
 * - **Streaming Final:** types.BankTransaction[]
 *
 * **Usage Patterns:**
 * 1. **Non‑streaming (Default)**
 *    - Best for quick responses and simple UI updates.
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useExtractTransactions({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useExtractTransactions({
 *   stream: true | undefined,
 *   onStreamData: (partial) => console.log('Partial update:', partial),
 *   onFinalData: (final) => console.log('Final result:', final),
 *   onError: (err) => console.error('Error:', err),
 ...
 * ```
 */
export function useExtractTransactions(props: HookInput<'ExtractTransactions', { stream: false }>): HookOutput<'ExtractTransactions', { stream: false }>
export function useExtractTransactions(props?: HookInput<'ExtractTransactions', { stream?: true }>): HookOutput<'ExtractTransactions', { stream: true }>
export function useExtractTransactions(
  props: HookInput<'ExtractTransactions', { stream?: boolean }> = {},
): HookOutput<'ExtractTransactions', { stream: true }> | HookOutput<'ExtractTransactions', { stream: false }> {
  let action: ServerAction = Actions.ExtractTransactions;
  if (isStreamingProps(props)) {
    action = StreamingActions.ExtractTransactions;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'ExtractTransactions', { stream: false }>)
  }
}