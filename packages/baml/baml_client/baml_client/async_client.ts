...
export class BamlAsyncClient {
  ...
  ): Promise<types.CreditScoreAnalysis> {
    try {
      ...
      if (options.onTick) {
        const stream = this.stream.AnalyzeBankStatement(
          transactions,
          __baml_options__
        );
      ...
      return raw.parsed(false) as types.CreditScoreAnalysis
    } catch (error) {
  ...
  async ExtractTransactions(
    ...
    try {
      ...
      // Check if onTick is provided - route through streaming if so
      if (options.onTick) {
        const stream = this.stream.ExtractTransactions(
          pdfText,
          __baml_options__
        );
      ...
      return raw.parsed(false) as types.BankTransaction[]
    } catch (error) {
      throw toBamlError(error);
    }
  }
...
class BamlStreamClient {
  ...
  AnalyzeBankStatement(
      transactions: types.BankTransaction[],
      __baml_options__?: BamlCallOptions
  ): BamlStream<partial_types.CreditScoreAnalysis, types.CreditScoreAnalysis> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const signal = options.signal;
        ...
        onTickWrapper = () => {
          ...
          if (log) {
            try {
              ...
              console.error("Error in onTick callback for AnalyzeBankStatement", error);
            }
          }
        };
      }
      ...
      return new BamlStream<partial_types.CreditScoreAnalysis, types.CreditScoreAnalysis>(
        raw,
        (a): partial_types.CreditScoreAnalysis => a,
        (a): types.CreditScoreAnalysis => a,
        this.ctxManager.cloneContext(),
        options.signal,
      )
    } catch (error) {
  ...
  ExtractTransactions(
      pdfText: string,
      __baml_options__?: BamlCallOptions
  ): BamlStream<partial_types.BankTransaction[], types.BankTransaction[]> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const signal = options.signal;
        ...
        onTickWrapper = () => {
          ...
          if (log) {
            try {
              ...
              console.error("Error in onTick callback for ExtractTransactions", error);
            }
          }
        };
      }
      ...
      return new BamlStream<partial_types.BankTransaction[], types.BankTransaction[]>(
        raw,
        (a): partial_types.BankTransaction[] => a,
        (a): types.BankTransaction[] => a,
        this.ctxManager.cloneContext(),
        options.signal,
      )
    } catch (error) {
